...
would overwrite CI's fuller result with a smaller one. If `data/manifest.json`
needs a change, patch it surgically with a one-off script and let the nightly
job reconcile.
...
**`npm run a11y`** — four checks in one browser session:
...
config/
  ...
  projects.config.ts  ** DEAD — imported by nothing; see AUDIT A1 **
